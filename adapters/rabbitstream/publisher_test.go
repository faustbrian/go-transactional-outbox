...
func TestPublisherConstructionRequiresClientOneTargetAndValidLimits(t *testing.T) {
	...
	if publisher, err := outboxrabbitstream.New(nil, outboxrabbitstream.Config{Stream: "events"}); publisher != nil || !errors.Is(err, outboxrabbitstream.ErrClientRequired) {
		t.Fatalf("nil client publisher/error = %#v/%v", publisher, err)
	}
	client := &recordingClient{}
	for name, config := range map[string]outboxrabbitstream.Config{
		"no target":   {},
		...
		t.Run(name, func(t *testing.T) {
			publisher, err := outboxrabbitstream.New(client, config)
			if publisher != nil || !errors.Is(err, outboxrabbitstream.ErrInvalidConfig) {
...
func TestPublisherMapsEnvelopeToOwnedConfirmedStreamMessage(t *testing.T) {
	...
	publisher, err := outboxrabbitstream.New(client, outboxrabbitstream.Config{Stream: "tracking.events"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublisherMapsSuperStreamAndStableRoutingFallbacks(t *testing.T) {
	...
	for name, envelope := range map[string]outbox.Envelope{
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			client := &recordingClient{result: rabbitstream.DeliveryResult{State: rabbitstream.DeliveryConfirmed}}
			publisher, err := outboxrabbitstream.New(client, outboxrabbitstream.Config{SuperStream: "tracking.events"})
			if err != nil {
				t.Fatal(err)
			}
...
func TestPublisherRequiresDefiniteConfirmationAndClassifiesRelayFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if class := outboxrabbitstream.ClassifyError(publishErr); class != test.class {
...
func TestClassifyErrorDistinguishesPermanentInputFailuresFromTransientOutcomes(t *testing.T) {
	...
	for name, err := range map[string]error{
		"invalid envelope":      outboxrabbitstream.ErrInvalidEnvelope,
		...
		t.Run(name, func(t *testing.T) {
			...
			if class := outboxrabbitstream.ClassifyError(err); class != relay.ErrorPermanent {
	...
	for name, err := range map[string]error{
		...
		t.Run(name, func(t *testing.T) {
			...
			if class := outboxrabbitstream.ClassifyError(err); class != relay.ErrorTransient {
...
func TestPublisherEnforcesConfiguredMessageBoundsBeforeClientAdmission(t *testing.T) {
	...
	client := &recordingClient{}
	publisher, err := outboxrabbitstream.New(client, outboxrabbitstream.Config{
...
func TestPublisherContainsClientPanicAsAmbiguousAndRejectsNilContext(t *testing.T) {
	...
	if !errors.Is(panicErr, rabbitstream.ErrPublishAmbiguous) ||
		outboxrabbitstream.ClassifyError(panicErr) != relay.ErrorTransient {
		t.Fatalf("panic error/class = %v/%v", panicErr, outboxrabbitstream.ClassifyError(panicErr))
	}
	...
	publisher, err = outboxrabbitstream.New(recording, outboxrabbitstream.Config{Stream: "tracking.events"})
	if err != nil {
		t.Fatal(err)
	}
	var nilContext context.Context
	if err := publisher.Publish(nilContext, envelope); !errors.Is(err, outboxrabbitstream.ErrContextRequired) {
