...
func TestPublisherMapsEnvelopeToKafkaMessage(t *testing.T) {
	...
	publisher, err := outboxkafka.New(client)
	if err != nil {
...
func TestPublisherEnforcesOutboxAndKafkaLimitsBeforePublish(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			client := &recordingClient{}
			publisher, err := outboxkafka.New(client, outboxkafka.WithLimits(limits))
			if err != nil {
				t.Fatal(err)
			}
...
func TestPublisherRejectsInvalidLimitsAndReservedMetadata(t *testing.T) {
	...
	client := &recordingClient{}
	invalid := outboxkafka.DefaultLimits()
	invalid.Kafka.MaxValueBytes = 0
	if publisher, err := outboxkafka.New(client, outboxkafka.WithLimits(invalid)); publisher != nil ||
	...
	if publisher, err := outboxkafka.New(client, nil); publisher != nil ||
		!errors.Is(err, outboxkafka.ErrInvalidConfig) {
		t.Fatalf("New() publisher/error = %#v/%v", publisher, err)
	}
	...
	for _, key := range []string{"content-type", "event-id", "schema-version", "idempotency-key"} {
		...
		if !errors.Is(err, outboxkafka.ErrReservedMetadata) {
...
func TestPublisherEnforcesEveryKafkaRecordBoundary(t *testing.T) {
	...
	tests := []struct {
		name   string
		adjust func(*outboxkafka.Limits, *outbox.Envelope)
		want   error
	}{
		{"topic", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxTopicBytes = 5 }, kafka.ErrTopicTooLarge},
		{"key", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxKeyBytes = 3 }, kafka.ErrKeyTooLarge},
		{"value", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxValueBytes = 3 }, kafka.ErrValueTooLarge},
		{"header count", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxHeaders = 2 }, kafka.ErrTooManyHeaders},
		{"header key required", func(_ *outboxkafka.Limits, envelope *outbox.Envelope) {
			envelope.Metadata = map[string]string{"": "value"}
		}, kafka.ErrHeaderKeyRequired},
		{"header key", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxHeaderKeyBytes = 13 }, kafka.ErrHeaderKeyTooLarge},
		{"header value", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxHeaderValueBytes = 15 }, kafka.ErrHeaderValueTooLarge},
		{"aggregate headers", func(limits *outboxkafka.Limits, _ *outbox.Envelope) { limits.Kafka.MaxHeaderBytes = 54 }, kafka.ErrHeadersTooLarge},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
...
func TestPublisherPreservesDeliveryCategoriesWithoutIdentityDisclosure(t *testing.T) {
	...
	for _, category := range categories {
		...
		t.Run(category.String(), func(t *testing.T) {
			...
			publisher, err := outboxkafka.New(&recordingClient{publishErr: cause})
			if err != nil {
				t.Fatal(err)
			}
			err = publisher.Publish(t.Context(), outbox.Envelope{
...
func TestClassifyErrorMapsKafkaFailuresToRelayPolicy(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := outboxkafka.ClassifyError(test.err); got != test.want {
...
func TestClassifyErrorTreatsRejectedEnvelopeAsPermanent(t *testing.T) {
	...
	if !errors.Is(publishErr, outboxkafka.ErrInvalidEnvelope) || client.calls != 0 {
		t.Fatalf("Publish() error/calls = %v/%d", publishErr, client.calls)
	}
	if got := outboxkafka.ClassifyError(publishErr); got != relay.ErrorPermanent {
...
func TestPublisherRedactsHealthFailureDiagnostic(t *testing.T) {
	...
	publisher, err := outboxkafka.New(&recordingClient{healthErr: cause})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublisherConvertsClientPanicToAmbiguousOutcome(t *testing.T) {
	...
	publisher, err := outboxkafka.New(panickingClient{})
	if err != nil {
		t.Fatal(err)
	}
	err = publisher.Publish(t.Context(), outbox.Envelope{
	...
	if !errors.Is(err, outboxkafka.ErrPublishPanic) ||
		!errors.As(err, &categorized) ||
		categorized.Category() != kafka.ErrorAmbiguous ||
		categorized.Error() != outboxkafka.ErrPublishPanic.Error() {
		t.Fatalf("Publish() error = %v", err)
	}
...
func TestPublisherRequiresClientAndPreservesFailures(t *testing.T) {
	...
	if publisher, err := outboxkafka.New(nil); publisher != nil ||
		!errors.Is(err, outboxkafka.ErrClientRequired) {
...
func TestPublisherRejectsEnvelopeWithoutRequiredRoutingIdentity(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.Is(err, outboxkafka.ErrInvalidEnvelope) || client.calls != 0 {
				t.Fatalf("Publish() error/calls = %v/%d", err, client.calls)
			}
		})
	}
