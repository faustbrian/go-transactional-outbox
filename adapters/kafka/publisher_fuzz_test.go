...
func FuzzPublisherEnvelopeMapping(f *testing.F) {
	...
	f.Fuzz(func(
		...
		limits := outboxkafka.DefaultLimits().Envelope
		...
		publisher, err := outboxkafka.New(first)
		if err != nil {
			t.Fatal(err)
		}
		...
		secondPublisher, err := outboxkafka.New(second)
		if err != nil {
			t.Fatal(err)
		}
		...
		failingPublisher, err := outboxkafka.New(&recordingClient{publishErr: cause})
		if err != nil {
			t.Fatal(err)
		}
